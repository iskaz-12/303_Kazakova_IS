...
CREATE TABLE IF NOT EXISTS workers (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, surname TEXT, name TEXT, patronymic TEXT, date_of_birth TEXT, specialization TEXT, number_of_directed_works INTEGER, number_of_completed_works INTEGER, percentage_of_revenue REAL, status TEXT, CHECK (number_of_directed_works>=0 AND number_of_completed_works>=0 AND percentage_of_revenue>0 AND (status = 'является работником' OR status = 'не является работником')));
CREATE TABLE IF NOT EXISTS busyness (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, data TEXT, work_hours TEXT, free_hours TEXT);
...
CREATE TABLE IF NOT EXISTS sign_up (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, time_of_record TEXT, service_execution_time TEXT, contact_phone_number TEXT DEFAULT NULL, completion_of_work TEXT, car_brand TEXT DEFAULT NULL, car_model TEXT DEFAULT NULL, CHECK(contact_phone_number LIKE '+%' AND (completion_of_work = 'да' OR completion_of_work = 'нет')));
CREATE TABLE IF NOT EXISTS sign_up_workers_services_car_categories (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL);
ALTER TABLE workers_busyness ADD COLUMN worker_id INTEGER REFERENCES workers (id) ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE sign_up_workers_services_car_categories ADD COLUMN sign_up_id INTEGER REFERENCES sign_up(id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE sign_up_workers_services_car_categories ADD COLUMN worker_id INTEGER REFERENCES workers(id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE sign_up_workers_services_car_categories ADD COLUMN services_car_categories_id INTEGER REFERENCES services_car_categories(id) ON DELETE RESTRICT ON UPDATE CASCADE;
INSERT INTO workers (surname, name, patronymic, date_of_birth, specialization, number_of_directed_works, number_of_completed_works, percentage_of_revenue, status)
VALUES
("Федотов", "Евгений", "Николаевич", "1985-04-15", "Автомеханик", 2, 2, 30, "является работником"),
("Архипов", "Юрий", "Викторович", "1990-07-22", "Автомеханик", 2, 2, 27, "является работником"),
("Соколов", "Сергей", "Андреевич", "1992-01-12", "Автомеханик", 1, 1, 26, "является работником"),
("Воробьёв", "Андрей", "Павлович", "1990-06-01", "Автомеханик", 1, 1, 27, "является работником"),
("Герасимов", "Пётр", "Анатольевич", "1987-10-10", "Автоэлектрик", 1, 1, 29, "является работником"),
("Паршутин", "Фёдор", "Сергеевич", "1983-12-05", "Автоэлектрик", 0, 0, 27, "является работником"),
("Венедиктов", "Константин", "Сергеевич", "1993-04-30", "Мастер кузовного ремонта", 1, 1, 32, "является работником"),
("Сидоров", "Павел", "Петрович", "1995-10-12", "Мастер кузовного ремонта", 0, 0, 25, "является работником"),
("Перфилов", "Андрей", "Анатольевич", "1990-05-02", "Мастер кузовного ремонта", 0, 0, 28, "является работником"),
("Бородин", "Михаил", "Игоревич", "1992-07-07", "Автомаляр", 1, 1, 31, "является работником"),
("Горин", "Геннадий", "Иванович", "1987-08-25", "Автожестянщик", 4, 4, 27, "является работником"),
("Устимов", "Пётр", "Иванович", "1985-09-20", "Автожестянщик", 2, 2, 31, "является работником");
